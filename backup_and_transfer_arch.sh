...
spool ${BFILE};
exec RDSADMIN.RDSADMIN_RMAN_UTIL.BACKUP_ARCHIVELOG_DATE( P_OWNER => 'SYS', P_DIRECTORY_NAME => '${BDIR}', P_FROM_DATE => to_date('11/03/2021 18:00:00','MM/DD/YYYY HH24:MI:SS'), P_TO_DATE => to_date('11/03/2021 19:25:00','MM/DD/YYYY HH24:MI:SS'), P_COMPRESS => ${COMPRESS}, P_PARALLEL => ${PARALLEL}, P_RMAN_TO_DBMS_OUTPUT => TRUE);
spool off;
EOF
...
retval=1
lcount=0
while [ ${retval} -eq 1 ] && [ ${lcount} -lt 200 ]
do
        if [ ${lcount} -gt 4 ]
        then
                let lsleep=${SSEC}*10
